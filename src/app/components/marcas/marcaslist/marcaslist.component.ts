...
export class MarcaslistComponent {
  ...
  listAll(){
    ...
    this.marcaService.listAll().subscribe({
      next: (lista: Marca[]) => { //quando o back retornar o que se espera
        this.lista = lista;
      },
      error: (erro: any) => { //quando ocorrer qualquer erro (badrequest, exceptions..)
        Swal.fire({
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.marcaService.delete(marca.id).subscribe({
          next: (mensagem: any) => { //quando o back retornar o que se espera
            Swal.fire({
              title: mensagem,
              icon: 'success',
              confirmButtonText: 'Ok',
